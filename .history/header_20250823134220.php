            ...
            class="nav-link dropdown-toggle <?php echo ($current_page == 'shop.php') ? 'active' : ''; ?>"
                ...
                <div class="mega-menu" id="categoryMenu">
                    <div class="d-flex flex-wrap">
                        <?php
                        // Fetch categories grouped by Main_Category_name (ignore empty)
                        $sql = "SELECT * FROM categories WHERE Main_Category_name IS NOT NULL AND Main_Category_name != '' ORDER BY Main_Category_name ASC, category_name ASC";
                        $result = mysqli_query($conn, $sql);

                        $categoriesByMain = [];
                        while ($row = mysqli_fetch_assoc($result)) {
                            $main = $row['Main_Category_name'];
                            $categoriesByMain[$main][] = $row;
                        }

                        foreach ($categoriesByMain as $mainCategory => $categories) {
                            echo '<div class="me-5">';
                            echo '<h5 style="font-weight:600; margin-bottom:10px;">' . htmlspecialchars($mainCategory) . '</h5>';
                            ...
                            foreach ($categories as $cat) {
                                ...
                                echo '
                        <li>
                            <a href="shop.php?category=' . urlencode($name) . '#product-list" class="text-dark text-decoration-none d-block">
                                <div class="img-bg mb-1" style="width:50px;height:50px;margin:0 auto;border-radius:6px;overflow:hidden;background:#f5f5f5;">
