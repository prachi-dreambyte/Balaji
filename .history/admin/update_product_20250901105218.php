...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    if (!empty($_POST['remove_image'])) {
        ...
        $imgs = array_values($imgs); // reindex after removals
    }


    /**
     * 2. Replace images (by index)
     */
    ...
    /**
     * 3. Reorder images (if order given)
     */
    if (!empty($_POST['image_order'])) {
        $order = explode(',', $_POST['image_order']); // file paths
        $new_imgs = [];
     ...
     */
    $imageOption = $_POST['image_option'] ?? ''; // fix undefined variable
