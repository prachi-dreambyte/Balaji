...
$sql = "
    SELECT 'Order Placed' AS action, order_id AS ref, created_at 
    FROM orders
    
    UNION ALL
    
    SELECT 'Product Uploaded' AS action, product_name AS ref, created_at
    FROM products
    
    UNION ALL
    
    SELECT 'Daily Deal Created' AS action, product_name AS ref, created_at
...
while ($row = $result->fetch_assoc()) {
    $activities[] = [
        ...
        "title" => $row['action'],
        "description" => $row['ref'],
        "time" => date("l, g:i A", strtotime($row['created_at']))
    ];
}
?>
    ...
    <div class="offcanvas offcanvas-end border-0" tabindex="-1" id="theme-activity-offcanvas"
        ...
        <div class="offcanvas-body p-0">
            <div data-simplebar class="h-100 p-4">
                <div class="position-relative ms-2" id="activity-timeline">
                    <span class="position-absolute start-0 top-0 border border-dashed h-100"></span>
                    <!-- Dynamic Timeline Items Will Render Here -->
                </div>
                <a href="#!" class="btn btn-outline-dark w-100 mt-3">View All</a>
            </div>
        </div>
    </div>
</div>
