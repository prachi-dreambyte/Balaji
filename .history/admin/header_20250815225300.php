...
// Fetch last 5 orders
$sql = "SELECT order_id, user_id, amount, status, created_at 
        FROM orders 
    ...
    <div style="max-height: 250px; overflow-y: auto;">
        ...
        if ($result->num_rows > 0) {
            while ($order = $result->fetch_assoc()) {
                echo '
                <a href="order-details.php?order_id=' . $order['order_id'] . '" class="dropdown-item">
                    <div class="d-flex align-items-center">
                        <div class="flex-grow-1">
                            <h6 class="mb-0">Order #' . $order['order_id'] . '</h6>
                            <small>User: ' . $order['user_id'] . ' | ₹' . $order['amount'] . '</small><br>
                            <small>Status: <strong>' . ucfirst($order['status']) . '</strong></small>
                        </div>
                        <small class="text-muted ms-2">' . date("d M, H:i", strtotime($order['created_at'])) . '</small>
                    </div>
                </a>
                ';
            }
