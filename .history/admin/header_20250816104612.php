...
if ($countResult = $conn->query($countQuery)) {
    ...
    error_log("Error fetching badge count: " . $conn->error);
}

// Initialize activities array
$activities = [];

// Activity query with error handling
$sql = "SELECT 'Order Placed' AS action, CONCAT('Order #', order_id) AS ref, created_at FROM orders
        UNION ALL SELECT 'Product Uploaded' AS action, product_name AS ref, created_at FROM products
        UNION ALL SELECT 'Daily Deal Created' AS action, product_name AS ref, created_at FROM home_daily_deal
        UNION ALL SELECT 'New Review Posted' AS action, CONCAT(user_name, ' rated ', rating, '★ - ', LEFT(review_text, 50), '...') AS ref, created_at FROM reviews
        UNION ALL SELECT 'Blog Published' AS action, title AS ref, created_at FROM blog
