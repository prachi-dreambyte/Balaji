<?php
if (!isset($pdo)) {
    require_once __DIR__ . '/../db_connect.php'; // adjust the path to match your folder structure
}
?>

// Count pending/unpaid orders for badge
$countQuery = "SELECT COUNT(*) AS total FROM orders WHERE status='pending' OR payment_status='Un paid'";
$countResult = $conn->query($countQuery);
$countRow = $countResult->fetch_assoc();
$badgeCount = $countRow['total'];

// Fetch latest 5 orders
$orderQuery = "SELECT order_id, user_id, amount, status, created_at 
               FROM orders 
        ...
        <?php if ($badgeCount > 0): ?>
                    ...
                    <div class="dropdown-menu py-0 dropdown-lg dropdown-menu-end" aria-labelledby="page-header-notifications-dropdown">
                        <div class="p-3 border-top-0 border-start-0 border-end-0 border-dashed border">
                            <div class="row align-items-center">
                                ...
                                <div class="col-auto">
                                    <a href="orders-list.php" class="text-dark text-decoration-underline">
                        ...
                        <div data-simplebar style="max-height: 280px;">
                            ...
                            if ($ordersResult->num_rows > 0) {
                                while ($order = $ordersResult->fetch_assoc()) {
                                    echo '
                    <a href="order-details.php?order_id=' . $order['order_id'] . '" class="dropdown-item py-3 border-bottom text-wrap">
                        <div class="d-flex">
                            ...
                            <div class="flex-grow-1">
                                <p class="mb-0 fw-semibold">Order #' . $order['order_id'] . '</p>
                                <p class="mb-0 text-wrap">
                                    User: ' . $order['user_id'] . ' | ₹' . $order['amount'] . '<br>
                                    Status: ' . ucfirst($order['status']) . '
                                </p>
                                <small class="text-muted">' . date("d M, h:i A", strtotime($order['created_at'])) . '</small>
                            </div>
                        </div>
                    </a>
                    ';
                                ...
                                echo '<p class="text-center p-3 mb-0">No new orders</p>';
                            }
                            ?>
                        </div>
                
                        <div class="text-center py-3">
                            <a href="orders-list.php" class="btn btn-primary btn-sm">
                                View All Orders <i class="bx bx-right-arrow-alt ms-1"></i>
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="dropdown topbar-item">
                    ...
                    <div class="dropdown-menu dropdown-menu-end">
                        ...
                        <a class="dropdown-item text-danger" href="sign_out.php">
...
<!-- Activity Timeline -->
<?php
$logs = $pdo->query("SELECT * FROM activity_log ORDER BY created_at DESC LIMIT 10")->fetchAll();
?>
...
<div class="dropdown topbar-item">
    <button type="button" class="topbar-button position-relative" data-bs-toggle="dropdown">
        ...
        <?php if (count($logs) > 0): ?>
            <span class="position-absolute topbar-badge translate-middle badge bg-danger rounded-pill">
                <?= count($logs) ?>
            </span>
        <?php endif; ?>
    </button>
    ...
    <div class="dropdown-menu dropdown-menu-end p-0" style="width: 350px;">
        ...
        <div style="max-height: 300px; overflow-y:auto;">
            <?php foreach ($logs as $log): ?>
                <a href="javascript:void(0);" class="dropdown-item py-2 border-bottom">
                    <small class="text-muted"><?= date('d M Y, h:i A', strtotime($log['created_at'])) ?></small>
                    <p class="mb-0"><?= htmlspecialchars($log['description']) ?></p>
                </a>
            <?php endforeach; ?>
        </div>
        <div class="text-center p-2">
            <a href="all-activities.php" class="btn btn-primary btn-sm">View All</a>
        </div>
    </div>
</div>
...
<script>
    const activities = [
        {
            type: 'icon',
            iconClass: 'iconamoon:folder-check-duotone',
            bgColor: 'bg-danger',
            title: 'Report-Fix / Update',
            description: `Add 3 files to <span class="text-primary"><iconify-icon icon="iconamoon:file-light"></iconify-icon> Tasks</span>`,
            files: [
                { icon: 'bx bxl-figma text-red', name: 'Concept.fig', link: '#!' },
                { icon: 'bx bxl-file-doc text-success', name: 'larkon.docs', link: '#!' }
            ],
            time: 'Monday, 4:24 PM'
        },
        {
            type: 'icon',
            iconClass: 'iconamoon:check-circle-1-duotone',
            bgColor: 'bg-success',
            title: 'Project Status',
            description: `Marked <span class="text-primary"><iconify-icon icon="iconamoon:file-light"></iconify-icon> Design</span> as <span class="badge bg-success-subtle text-success">Completed</span>`,
            files: [{ name: 'UI/UX Figma Design', link: '#!' }],
            time: 'Monday, 3:00 PM'
        },
        {
            type: 'text',
            initials: 'UI',
            bgColor: 'bg-primary',
            title: 'Larkon Application UI v2.0.0 <span class="badge bg-primary-subtle text-primary">Latest</span>',
            description: 'Get access to over 20+ pages including dashboard, charts, kanban board, and more.',
            action: '<a href="#!" class="btn btn-light btn-sm">Download Zip</a>',
            time: 'Monday, 2:10 PM'
        },
        {
            type: 'image',
        ...
        },
        {
            ...
            description: '<iconify-icon icon="iconamoon:check-circle-1-duotone" class="text-success"></iconify-icon> Added a new member to Front Dashboard',
            time: 'Monday, 10:00 AM'
        },
        {
            type: 'icon',
            iconClass: 'iconamoon:certificate-badge-duotone',
            bgColor: 'bg-warning',
            title: 'Achievements',
            description: '<iconify-icon icon="iconamoon:certificate-badge-duotone" class="text-danger fs-20"></iconify-icon> "Best Product Award"',
            time: 'Monday, 9:30 AM'
        }
    ];
    ...
    activities.forEach(activity => {
        let iconHtml = '';
        if (activity.type === 'icon') {
            iconHtml = `<span class="position-absolute start-0 avatar-sm translate-middle-x ${activity.bgColor} d-inline-flex align-items-center justify-content-center rounded-circle text-light fs-20">
                            <iconify-icon icon="${activity.iconClass}"></iconify-icon>
                        </span>`;
        } else if (activity.type === 'text') {
            iconHtml = `<span class="position-absolute start-0 avatar-sm translate-middle-x ${activity.bgColor} d-inline-flex align-items-center justify-content-center rounded-circle text-light fs-16">
                            ${activity.initials}
                        </span>`;
        } else if (activity.type === 'image') {
            iconHtml = `<span class="position-absolute start-0 translate-middle-x ${activity.bgColor || ''} d-inline-flex align-items-center justify-content-center rounded-circle text-light fs-20">
                            <img src="${activity.avatar}" class="avatar-sm rounded-circle">
                        </span>`;
        }

        let contentHtml = `<div class="ms-2">
        ...
        if (activity.files) {
            contentHtml += `<div class="bg-light bg-opacity-50 rounded-2 p-2"><div class="row">`;
            activity.files.forEach(file => {
                contentHtml += `<div class="col-lg-6">
                    <div class="d-flex align-items-center gap-2">
                        ${file.icon ? `<i class="${file.icon} fs-20"></i>` : ''}
                        <a href="${file.link}" class="text-dark fw-medium">${file.name}</a>
                    </div>
                </div>`;
            });
            contentHtml += `</div></div>`;
        ...
        if (activity.images) {
            contentHtml += `<div class="row g-2 mt-2">`;
            activity.images.forEach(img => {
                contentHtml += `<div class="col-lg-4">
                    <a href="#!"><img src="${img}" class="img-fluid rounded"></a>
                </div>`;
            });
            contentHtml += `</div>`;
        }
        ...
        if (activity.action) {
            contentHtml += `<div class="mt-2">${activity.action}</div>`;
        }

        contentHtml += `<h6 class="mt-3 text-muted">${activity.time}</h6></div>`;

        container.innerHTML += `<div class="position-relative ps-4 mb-4">
            ${iconHtml}
            ${contentHtml}
        </div>`;
    });
...
<div class="main-nav">
    ...
    <div class="scrollbar" data-simplebar>
        <ul class="navbar-nav" id="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#sidebarCategory1" data-bs-toggle="collapse" role="button" aria-expanded="false" aria-controls="sidebarCategory1">
                    <span class="nav-icon">
                        <iconify-icon icon="solar:clipboard-list-bold-duotone"></iconify-icon>
                    </span>
                ...
                <div class="collapse" id="sidebarCategory1">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item">
                            <a class="sub-nav-link" href="coupon-list.php">List</a>
                        </li>
                        <li class="sub-nav-item">
                            <a class="sub-nav-link" href="coupon-edit.php">Edit</a>
                        </li>
                        <li class="sub-nav-item">
                            <a class="sub-nav-link" href="coupon-add.php">Create</a>
                        </li>
                    </ul>
                </div>
            </li>
