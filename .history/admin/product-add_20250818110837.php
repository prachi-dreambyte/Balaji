...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Collect form data (same as your code)
    ...
    // Image upload handling (same as your code)
    $imageArray = [];
    $uploadDir = "uploads/";
    ...
    if ($stmt->num_rows > 0) {
        ...
        // Insert product first (with NULL variant if not provided)
        $sql = "INSERT INTO products 
            (product_name, category, brand, weight, size, short_description, description, tag_number, stock, tags, price, discount, corporate_discount, tax, images, variants, colour) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = $conn->prepare($sql);
        $stmt->bind_param(
            "ssssssssissddssss",
            $product_name, $category, $brand, $weight, $size, $short_description, $description, 
            $tag_number, $stock, $tags, $price, $discount, $corporate_discount, $tax, 
            $imagesJSON, $variants, $colour
        );
