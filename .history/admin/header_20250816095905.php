...
if ($countResult = $conn->query($countQuery)) {
    $countRow = $countResult->fetch_assoc();
    $badgeCount = $countRow['total'];
} else {
    $errorMsg = "Error fetching badge count: " . $conn->error;
}

// Activity query
$sql = "
    SELECT 'Order Placed' AS action, CONCAT('Order #', order_id) AS ref, created_at
    FROM orders
    UNION ALL
    ...
    UNION ALL
    SELECT 'New Review Posted' AS action, CONCAT(user_name, ' rated ', rating, '★ - ', LEFT(review_text, 50), '...') AS ref, created_at
    FROM reviews
    UNION ALL
    SELECT 'Blog Published' AS action, title AS ref, created_at
    FROM blog
...
if ($result = $conn->query($sql)) {
    while ($row = $result->fetch_assoc()) {
        $activities[] = [
            'action' => $row['action'],
            'ref' => $row['ref'],
            'time' => date("l, d M Y h:i A", strtotime($row['created_at']))
        ];
    }
} else {
    $errorMsg = "Error fetching activities: " . $conn->error;
}


// Fetch latest 5 orders
$orderQueryy = "SELECT order_id, user_id, amount, status, created_at 
               FROM orders 
               ORDER BY created_at DESC 
               LIMIT 5";
$ordersResults = $conn->query($orderQueryy);
?>
<header class="topbar">
                    ...
                    <div class="dropdown topbar-item">
                         ...
                         <div class="dropdown-menu py-0 dropdown-lg dropdown-menu-end"
                              ...
                              <div data-simplebar style="max-height: 280px;">
                                   <?php
                                   if ($ordersResults->num_rows > 0) {
                                       while ($orderss = $ordersResults->fetch_assoc()) {
                                           echo '
                    <a href="order-details.php?order_id=' . $orderss['order_id'] . '" class="dropdown-item py-3 border-bottom text-wrap">
                        <div class="d-flex">
                            ...
                            <div class="flex-grow-1">
                                <p class="mb-0 fw-semibold">Order #' . $orderss['order_id'] . '</p>
                                <p class="mb-0 text-wrap">
                                    User: ' . $orderss['user_id'] . ' | ₹' . $orderss['amount'] . '<br>
                                    Status: ' . ucfirst($orderss['status']) . '
                                </p>
                                <small class="text-muted">' . date("d M, h:i A", strtotime($orderss['created_at'])) . '</small>
                            </div>
                        </div>
                    </a>';
                                       }
...
<div class="offcanvas offcanvas-end border-0" tabindex="-1" id="theme-activity-offcanvas"
     ...
     <div class="offcanvas-body p-0">
          <div data-simplebar class="h-100 p-4">
               ...
               <button id="loadMoreBtn" class="btn btn-outline-dark w-100 mt-3">View More</button>
          </div>
     </div>
</div>
...
<script>
     ...
     function renderActivities() {
          if (!Array.isArray(activities)) {
               container.innerHTML = `<div class="text-danger fw-bold">Error loading activities</div>`;
               loadMoreBtn.style.display = "none";
               return;
          }

          let end = visibleCount + batchSize;
          let batch = activities.slice(visibleCount, end);

          batch.forEach(activity => {
               let iconHtml = `<span class="position-absolute start-0 avatar-sm translate-middle-x bg-primary 
                                d-inline-flex align-items-center justify-content-center rounded-circle text-light fs-20">
                                    <i class="bi bi-bell"></i>
                                </span>`;

               let contentHtml = `<div class="ms-2">
                    <h5 class="mb-1 text-dark fw-semibold fs-15 lh-base">${activity.action}</h5>
                    ${activity.ref ? `<p class="mb-2">${activity.ref}</p>` : ''}
                    <h6 class="mt-3 text-muted">${activity.time}</h6>
                </div>`;
          ...
          if (visibleCount >= activities.length) {
               loadMoreBtn.innerText = "No more activity";
     ...
     loadMoreBtn.addEventListener('click', renderActivities);
</script>
    ...
    width: 210px;" alt="logo light">
     ...
     <div class="scrollbar" data-simplebar>
          <ul class="navbar-nav" id="navbar-nav">
               ...
               <li class="nav-item">
                    <a class="nav-link menu-arrow" href="#sidebarCategory1" data-bs-toggle="collapse" role="button"
                         aria-expanded="false" aria-controls="sidebarCategory1">
                         <span class="nav-icon">
                              <iconify-icon icon="solar:clipboard-list-bold-duotone"></iconify-icon>
                         </span>
               ...
               <li class="nav-item">
                    <a class="nav-link menu-arrow" href="#sidebarHomeBanners" data-bs-toggle="collapse" role="button"
                         aria-expanded="false" aria-controls="sidebarHomeBanners">
                         <span class="nav-icon">
                              <iconify-icon icon="solar:bag-smile-bold-duotone"></iconify-icon>
                         </span>
               ...
               <li class="nav-item">
                    <a class="nav-link menu-arrow" href="#sidebarHome-blogs" data-bs-toggle="collapse" role="button"
                         aria-expanded="false" aria-controls="sidebarHome-blogs">
                         <span class="nav-icon">
                              <iconify-icon icon="solar:bag-smile-bold-duotone"></iconify-icon>
                         </span>
               ...
               <li class="nav-item">
                    <a class="nav-link menu-arrow" href="#sidebarHome-brands" data-bs-toggle="collapse" role="button"
                         aria-expanded="false" aria-controls="sidebarHome-brands">
                         <span class="nav-icon">
                              <iconify-icon icon="solar:bag-smile-bold-duotone"></iconify-icon>
                         </span>
               ...
               <li class="nav-item">
                    <a class="nav-link menu-arrow" href="#sidebarHome-product" data-bs-toggle="collapse" role="button"
                         aria-expanded="false" aria-controls="sidebarHome-product">
                         <span class="nav-icon">
                              <iconify-icon icon="solar:bag-smile-bold-duotone"></iconify-icon>
                         </span>
               ...
               <li class="nav-item">
                    <a class="nav-link menu-arrow" href="#sidebarHome-services" data-bs-toggle="collapse" role="button"
                         aria-expanded="false" aria-controls="sidebarHome-services">
                         <span class="nav-icon">
                              <iconify-icon icon="solar:bag-smile-bold-duotone"></iconify-icon>
                         </span>
               ...
               <li class="nav-item">
                    <a class="nav-link menu-arrow" href="#sidebarHome-testimonials" data-bs-toggle="collapse"
                         role="button" aria-expanded="false" aria-controls="sidebarHome-testimonials">
                         <span class="nav-icon">
                              <iconify-icon icon="solar:bag-smile-bold-duotone"></iconify-icon>
                         </span>
