...
// NOTE: $types must be a STRING (e.g., $types = '') before this block.
if (!empty($search_term)) {
    // normalize
    $q = trim(strtolower($search_term));

    // optional noise words / currency markers often used in India
    $q = preg_replace('/\b(rs|₹|inr|price)\b/i', '', $q);
    $q = trim($q);

    // 1) Handle "under/below/less than/up to/upto" + optional Rs/₹
    if (preg_match('/\b(under|below|less\s*than|up\s*to|upto)\s*(rs|₹)?\s*(\d{2,})\b/i', $q, $m)) {
        $price_limit = (int)$m[3];
        $conditions[] = "price <= ?";
        $params[] = $price_limit;
        $types .= 'i';
        // strip the matched phrase from keyword text
        $q = trim(str_ireplace($m[0], '', $q));
    }

    // 2) Numeric-only search => treat as budget ceiling (e.g., "2000")
    if ($q !== '' && ctype_digit($q)) {
        $conditions[] = "price <= ?";
        $params[] = (int)$q;
        $types .= 'i';
        $q = ''; // don't also search as text
    }

    // 3) Keyword LIKE search (if anything left)
    if ($q !== '') {
        $like = "%$q%";
