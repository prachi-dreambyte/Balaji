		...
		<div class="container">
          ...
          WHERE product_id = $product_id";
					...
					if ($result && mysqli_num_rows($result) > 0) {
						...
						$avg_rating = round($reviewData['avg_rating'], 1); // keep one decimal
						$review_count = (int) ($reviewData['review_count'] ?? 0);
					} else {
						$avg_rating = 0;
						$review_count = 0;
					}

					// Function to display stars with half support
					function renderStars($rating)
					{
						...
						$stars = str_repeat('<span style="color:#FFD700;">★</span>', $fullStars);
						if ($halfStar) {
							$stars .= '<span style="color:#FFD700;">☆</span>'; // you can replace with a half-star icon if using Font Awesome
						}
						$stars .= str_repeat('<span style="color:#ccc;">★</span>', $emptyStars);
					...
					<div class="abc">
						...
						<!-- ⭐ Rating & Review Count Amazon Style -->
						<div class="product-rating" style="margin:6px 0; font-size:15px; color:#333;">
							<?php if ($review_count > 0): ?>
								<span class="avg-rating" style="font-weight:600;"><?= $avg_rating; ?></span>
								<span class="stars"><?= renderStars($avg_rating); ?></span>
								<span class="review-count" style="margin-left:8px; color:#666;">
									<?= number_format($review_count); ?> ratings
								</span>
							<?php else: ?>
								<span class="no-reviews" style="color:#999;">No ratings yet</span>
							<?php endif; ?>
						</div>
