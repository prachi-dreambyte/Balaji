...
// NOTE: $types must be initialized as '' before this block.
if (!empty($search_term)) {
	// Normalize input
	$q = trim(strtolower($search_term));

	// Remove noise words / currency markers often used in India
	$q = preg_replace('/\b(rs|₹|inr|rupees|price|cost)\b/i', '', $q);
	$q = preg_replace('/\/-/', '', $q); // remove "/-"
	$q = preg_replace('/,/', '', $q);   // remove commas in numbers
	$q = trim($q);

	// 1) Handle BETWEEN queries like "between 1000 and 5000"
	if (preg_match('/between\s*(\d+)\s*(and|to|-)\s*(\d+)/i', $q, $m)) {
		$min_price = (int) $m[1];
		$max_price = (int) $m[3];
		...
		$q = trim(str_ireplace($m[0], '', $q));
	}

	// 2) Handle "under/below/less than/up to/upto" + Rs/₹
	if (preg_match('/\b(under|below|less\s*than|up\s*to|upto)\s*(rs|₹)?\s*(\d{2,})\b/i', $q, $m)) {
		$price_limit = (int) $m[3];
		$conditions[] = "price <= ?";
		$params[] = $price_limit;
		$types .= 'i';
	...
	// 3) Handle "above/greater than/more than/over"
	if (preg_match('/\b(above|greater\s*than|more\s*than|over)\s*(rs|₹)?\s*(\d{2,})\b/i', $q, $m)) {
		$min_price = (int) $m[3];
		$conditions[] = "price >= ?";
		$params[] = $min_price;
		$types .= 'i';
	...
	// 4) Numeric-only search => budget ceiling
	if ($q !== '' && ctype_digit($q)) {
		$conditions[] = "price <= ?";
		$params[] = (int) $q;
		$types .= 'i';
		$q = ''; // don't use it again as text
	}

	// 5) Keyword LIKE search
	if ($q !== '') {
		$like = "%$q%";
