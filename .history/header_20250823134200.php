...
.mega-menu {
    ...
    white-space: nowrap; /* auto width expansion */
}
        ...
        <a href="shop.php"
            class="nav-link dropdown-toggle <?php echo ($current_page == 'shop.php') ? 'active' : ''; ?>"
                ...
                <div class="mega-menu" id="categoryMenu"
                    style="background:#fff; padding:20px; border-radius:8px; box-shadow:0px 4px 12px rgba(0,0,0,0.15);">
                    <div class="row g-4">
                        <?php
                        // Fetch categories grouped by Main_Category_name
                        $sql = "SELECT * FROM categories ORDER BY Main_Category_name ASC, category_name ASC";
                        $result = mysqli_query($conn, $sql);

                        $categoriesByMain = [];
                        while ($row = mysqli_fetch_assoc($result)) {
                            $main = $row['Main_Category_name'] ?: 'Other'; // Fallback if empty
                            $categoriesByMain[$main][] = $row;
                        }

                        foreach ($categoriesByMain as $mainCategory => $categories) {
                            echo '<div class="col-md-3">';
                            echo '<h5 style="font-weight:bold; border-bottom:1px solid #ddd; padding-bottom:5px; margin-bottom:10px;">' . htmlspecialchars($mainCategory) . '</h5>';
                            echo '<ul class="list-unstyled">';

                            foreach ($categories as $cat) {
                                $name = htmlspecialchars($cat['category_name']);
                                $imageFile = $cat['category_image'];
                                ...
                                echo '
                        <li class="mb-2">
                            <a href="shop.php?category=' . urlencode($name) . '#product-list" class="d-flex align-items-center text-dark text-decoration-none">
                                <div class="img-bg me-2" style="width:40px;height:40px;overflow:hidden;border-radius:6px;">
                                    <img src="' . $imagePath . '" alt="' . $name . '" style="width:100%;height:100%;object-fit:cover;">
                                </div>
                                <span>' . $name . '</span>
                            </a>
                        </li>';
