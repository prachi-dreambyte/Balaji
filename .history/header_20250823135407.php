...
.mega-menu {
  ...
  top: 100%;
  left: 0;
  /* transparent box – no background or border */
...
/* --- Inner grid that holds all columns --- */
.mega-grid {
  display: flex;
  align-items: flex-start;
  gap: 28px;
  padding: 12px 8px;
  /* keep transparent so it feels like it belongs to the header */
  background: transparent;
}
...
.mega-col-title {
  ...
  margin: 0 0 8px 0;
  line-height: 1.2;
  /* no box, no bg: stays blank & transparent */
  background: transparent;
  padding: 0;
  color: #222;
}
...
/* links show only names with subtle hover */
.mega-link {
  display: block;
  text-decoration: none;
  color: #333;
...
/* Make sure the menu doesn’t get clipped by parent containers */
.header-section, .header-section * { overflow: visible !important; }

/* Optional: keep dropdown within the main container padding visually */
@media (min-width: 1200px) {
  .nav-item-with-mega .mega-menu {
    left: 0; /* anchored to the CATEGORY item */
  }
}

/* Touch friendly: increase tap target on mobile if you reuse this menu for mobile */
  ...
  <p class="headerText">
    <a href="shop.php"
       class="nav-link dropdown-toggle <?php echo ($current_page == 'shop.php') ? 'active' : ''; ?>"
            id="categoryLink" aria-haspopup="true" aria-expanded="false">
            CATEGORY
        </a>
    </p>

    <div class="mega-menu" id="categoryMenu" role="menu" aria-labelledby="categoryLink">
        <div class="mega-grid">
            <?php
            $sql = "SELECT id, category_name, Main_Category_name, category_image 
              FROM categories
              WHERE COALESCE(Main_Category_name,'') <> ''
              ORDER BY Main_Category_name ASC, category_name ASC";
            $result = mysqli_query($conn, $sql);

            $byMain = [];
            if ($result) {
                while ($r = mysqli_fetch_assoc($result)) {
                    $byMain[$r['Main_Category_name']][] = $r;
                }
            }

            foreach ($byMain as $main => $cats): ?>
                <div class="mega-col">
                    <div class="mega-col-title"><?php echo htmlspecialchars($main); ?></div>
                    <ul class="mega-list">
                        <?php foreach ($cats as $c): ?>
                            <li>
                                <a class="mega-link"
                                    href="shop.php?category=<?php echo urlencode($c['category_name']); ?>#product-list">
                                    <!-- Show image + name -->
                                    <?php if (!empty($c['category_image'])): ?>
                                        <img src="<?php echo htmlspecialchars($c['category_image']); ?>"
                                            alt="<?php echo htmlspecialchars($c['category_name']); ?>" class="mega-thumb">
                                    <?php endif; ?>
...
<script>
document.addEventListener('DOMContentLoaded', function () {
  const trigger = document.getElementById('categoryLink');
  const menu = document.getElementById('categoryMenu');
  const wrapper = trigger.closest('.nav-item-with-mega');

  if (!trigger || !menu || !wrapper) return;

  // Desktop: click toggles; hover already handled by CSS
  trigger.addEventListener('click', function (e) {
    // prevent navigating to shop.php when using dropdown
    if (window.innerWidth >= 1200) {
      e.preventDefault();
      const open = menu.style.display === 'inline-block';
      menu.style.display = open ? 'none' : 'inline-block';
