...
<?php
$search_term = isset($_GET['search']) ? trim(strtolower($_GET['search'])) : '';
$conditions = [];
$params = [];
$types = '';
...
if (!empty($search_term)) {
    ...
    // ✅ Common Indian search patterns
    // Remove "rs", "₹", "price", "ka", "ki", "ke"
    $search_term = preg_replace('/\b(rs|₹|price|ka|ki|ke|wala|with|without)\b/i', '', $search_term);
    $search_term = trim($search_term);

    // ✅ Handle "under/below/less than" searches
    if (preg_match('/(under|below|less than)\s*(rs|₹)?\s*(\d+)/i', $search_term, $matches)) {
        $price_limit = (int) $matches[3];
        $conditions[] = "price <= ?";
        $params[] = $price_limit;
        $types .= 'i';

        // remove the numeric price part from keyword search
        $search_term = preg_replace('/(under|below|less than)\s*(rs|₹)?\s*\d+/i', '', $search_term);
        $search_term = trim($search_term);
    }

    // ✅ Handle "numeric only" searches (like 2000 = under 2000)
    if (is_numeric($search_term)) {
        $conditions[] = "price <= ?";
        $params[] = (int) $search_term;
        $types .= 'i';
        $search_term = ''; // don't search as text
    }

    // ✅ Synonym mapping (Indian style)
    $synonyms = [
        'visitor chair' => ['client chair', 'guest chair', 'cardroom chair'],
        'student chair' => ['school chair', 'college chair', 'coaching chair'],
        'staff chair'   => ['office chair', 'work chair'],
    ];
    foreach ($synonyms as $main => $alts) {
        foreach ($alts as $alt) {
            if (strpos($search_term, $alt) !== false) {
                $search_term = str_replace($alt, $main, $search_term);
            }
        }
    }

    // ✅ Keyword search (if anything left)
    if (!empty($search_term)) {
        $like = "%$search_term%";
